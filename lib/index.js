...
function verifyAsync(token, secret, opts) {
...
module.exports = function(opts) {
  ...
  var middleware = function jwt(ctx, next) {
    ...
    const revocationHandler = (user) => revoked => {
      if (revoked) {
        return Promise.reject(new Error('Revoked token'));
      }
      return Promise.resolve(user)
    }

    const isRevoked = opts.isRevoked
      ? user => opts.isRevoked(ctx, user, token).then(revocationHandler(user))
      : user => user;
